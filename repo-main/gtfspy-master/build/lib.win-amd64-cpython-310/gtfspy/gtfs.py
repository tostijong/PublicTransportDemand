...
class GTFS(object):
    ...
    def get_transit_events(self, start_time_ut=None, end_time_ut=None, route_type=None):
        """
        RENZO! Obtain a list of events that take place during a time interval.
        Each event needs to be only partially overlap the given time interval.
        Does not include walking events.
        ...
        event_query = "SELECT stop_I, seq, trip_I, route_I, routes.route_id AS route_id, routes.type AS route_type, " \
                      "shape_id, direction_id, headsign, day_start_ut+dep_time_ds AS dep_time_ut, day_start_ut+arr_time_ds AS arr_time_ut " \
                      "FROM " + table_name + " " \
        ...
        direction_ids = events_result['direction_id'][from_indices]
        headsigns = events_result['headsign'][from_indices]
        ...
        data_tuples = zip(from_stops, to_stops, dep_times, arr_times,
                          shape_ids, direction_ids, headsigns, route_types, route_ids, trip_Is,
                          durations, from_seqs, to_seqs, route_Is)
        columns = ["from_stop_I", "to_stop_I", "dep_time_ut", "arr_time_ut",
                   "shape_id", "direction_id", "headsign", "route_type", "route_id", "trip_I",
