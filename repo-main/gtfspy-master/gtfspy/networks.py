...
DEFAULT_STOP_TO_STOP_LINK_ATTRIBUTES = [
    ...
    "d", "route_I_counts", "shape_id", "direction_id", "headsign"
]
...
def stop_to_stop_network_for_route_type(gtfs,
    ...
    for key, link_events in link_event_groups:
        ...
        if link_attributes is None:
            ...
            link_data = {}
            if "shape_id" in link_attributes:
            	link_data["shape_id"] = link_events.groupby("shape_id").size().to_dict()
            if "direction_id" in link_attributes:
            	link_data["direction_id"] = link_events.groupby("direction_id").size().to_dict()
            if "headsign" in link_attributes:
            	link_data["headsign"] = link_events.groupby("headsign").size().to_dict()
